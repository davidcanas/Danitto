...
export default class InteractionCreate {
	...
	async run(message: Message) {
		...
		if (!command) {
			...
			const filter = (r: Emoji, user: User) => r.name === '👍' && user === message.author;
			const collector = new ReactionCollector(this.client, mensagem, filter, { max: 1 });
		...
		if (command) {
			...
			if (user && user.blacklist) {
				...
				.setDescription("❌ Estás na minha blacklist então não podes usar mais comandos , caso aches injusto [clica aqui](https://dink.ga/blacklistDanitto) !")
				.setColor("RANDOM")
