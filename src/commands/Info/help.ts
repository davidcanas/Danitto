	...
	async execute(ctx: CommandContext): Promise<void> {
		...
		if (!args) {
			...
			const help = new this.client.embed()
				...
				.setDescription(`<:terminal:830124045701087252> Ao todo tenho ${this.client.commands.length - Owner.length} comandos!`)
				//.setImage("https://i.imgur.com/nJekMMr.jpg")
				.setImage("https://i.imgur.com/aCGhlv1.png")
				.setFooter("Usa d/help <comando> para mais ajuda!")
				.setColor("RANDOM")
			if (Info.length !== 0) {
				help.addField(`**:information_source: Informação [${Info.length}]**`, `\`\`\`${Info.join(" - ")}\`\`\``)
			}
			if (Util.length !== 0) {
				help.addField(`:bulb: Utilidades [${Util.length}]`, `\`\`\`${Util.join(" - ")}\`\`\``)
			}
			if (Fun.length !== 0) {
				help.addField(`:tada: Diversão [${Fun.length}]`, `\`\`\`${Fun.join(" - ")}\`\`\``)
			}
			if (Music.length !== 0) {
				help.addField(`🎵 Música [${Music.length}]`, `\`\`\`${Music.join(" - ")}\`\`\``)
			}
			ctx.sendMessage({
				embeds: [help],
				content: "",
			...
			const command = this.client.commands.find(c => c.name === args) || this.client.commands.find(c => c.aliases && c.aliases.includes(args));
			if (!command) {
