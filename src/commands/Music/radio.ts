...
export default class PlayRadio extends Command {
    ...
    async execute(ctx: CommandContext): Promise<void> {
        ...
        let array = ["oceanopacifico","renascenca","comercial","cidadefm",  "smoothfm",  "vodafone",   "tsf", "m80", "rfm" ]
            if(!ctx.args[0]) {
                ctx.sendMessage(`Rádios disponiveis: \`${array.join(",")}\``)
            return;
            }
            const res = await this.client.music.search(`https://m.danitto.live/${ctx.args[0]}`);

            if (res.loadType !== 'TRACK_LOADED') {
              ctx.sendMessage({ content: `Houve um problema ao tocar a rádio solicitada\nRádios disponiveis: \`${array.join(",")}\``, flags: 1 << 6 });
              player.destroy();
              return;
            }
