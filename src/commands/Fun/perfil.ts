...
export default class Profile extends Command {
      ...
      options: [
        {
          ...
          type: 3,
          description:
            "Caso queira ver o perfil de um usuario diferente de você.",
          required: false,
        },
      ],
    });
  ...
  async execute(ctx: CommandContext): Promise<void> {
    ...
    if (!verificado) {
      ...
      embed.setTitle(
        `Perfil de ${
          this.client.users.get(user.id).username
        } <:usuario_verificado_pela_equipe:942515716572213309>`
      );
      embed.setFooter(
        "Este usuario foi verificado e aprovado pelo CanasDev Verificações TM"
      );
    }
