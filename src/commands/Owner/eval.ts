...
export default class Eval extends Command {
	...
	async execute(ctx: CommandContext): Promise<void> {
		...
		try {
			...
			if (ctx.author.id !== "733963304610824252" && ctx.author.id !== "477534823011844120" && ctx.author.id !== "718078381199065150" && ctx.author.id !== "852650555254767676" && ctx.author.id !== "334054158879686657") {
			...
			if (code.includes(process.env.DANITOKEN) || code.includes(process.env.DANITOKEN2) || code.includes(process.env.MONGODB)) {
			...
			if (code.length > 1750) {
				const bin = await ctx.createBin(require("sourcebin"), code, "javascript")
				ctx.sendMessage(`Como o codigo passou dos 1800 caracteres envio um link do sourcebin: ${bin.short}\n||(Tempo de Execução: ${((stop[0] * 1e9) + stop[1]) / 1e6}ms )||`)
