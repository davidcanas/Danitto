...
export default class Render extends Command {
  ...
  async execute(ctx: CommandContext): Promise<void> {
    ...
    if(ctx.channel.nsfw || this.client.allowedUsers.includes(ctx.author.id)) {
      ...
      let args = ctx.args[0]
      if(!args) {
        ctx.sendMessage("Precisas de explicitar o link do website a renderizar")
        return;
    }
        ...
        const fetch = await this.client.fetch(`https://image.thum.io/get/maxAge/12/width/800/crop/1600/${ctx.args[0]}`)
        if(fetch.text() === "Invalid url in thumbnail request.") {
         if(ctx.type !== Type.INTERACTION) {
